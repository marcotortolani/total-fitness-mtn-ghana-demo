...
import { useExerciseStore } from '@/lib/training/training-stores'

import dictionary from '@/dictionary/lang.json'

export const ExerciseSectionItem = ({ section, training }) => {
      ...
      {open &&
        section?.exercises?.map((exercise, i) => {
          const prevExerciseCompleted = i
            ...
            : false
          const sectionCompleted = training.sections.filter(
            (section) => section.completed,
          )

          return (
            <ButtonExercise
              key={`${section.id}-${exercise.id}-${i}`}
              name={exercise?.title?.rendered}
              time={60 + (training?.level - 1) * 15}
              state={
